...
SELECT
  ...
  -- Financial ratios (unused downstream but retained if needed)
  ...
  score_lta,
  possible_lta,
  score_program_expense,
  possible_program_expense,
  score_wkgcap,
  possible_wkgcap,
  score_independent_board_size,
  possible_independent_board_size,
  score_independent_board,
  possible_independent_board
...
form990_scores <- form990_metrics %>%
  mutate(
    ...
    program_exp_score = if_else(
      is_eligible_program_expense,
      score_program_expense / possible_program_expense,
      NA_real_
    ),
    ...
    is_eligible_financial_statements =
      rating_size %in% c("SMALL", "MEDIUM", "LARGE", "SUPER"),
    
    financial_statements_score = case_when(
      rating_size == "SMALL" &
        (has_compilation_or_review | has_audit) ~ 1,
      rating_size %in% c("MEDIUM", "LARGE", "SUPER") &
        has_audit ~ 1,
      TRUE ~ 0
    ),
    
    is_eligible_audit_committee = case_when(
      rating_size == "SUPER" & is_donor_supported ~ TRUE,
      TRUE ~ FALSE
    ),
    
    audit_committee_score = if_else(
      is_eligible_audit_committee,
      as.numeric(has_audit_committee),
      NA_real_
    ),
    ...
    document_retention_policy_score = if_else(
      is_eligible_document_retention_policy,
      as.numeric(has_document_retention_policy),
      NA_real_
    ),
    
    form990_on_website_score = if_else(
      is_eligible_990_on_website,
      as.numeric(has_990_on_website),
      NA_real_
    ),
    ...
    independent_board_size_score = if_else(
      is_eligible_independent_board_size,
      score_independent_board_size / possible_independent_board_size,
      NA_real_
    ),
    
    independent_board_score = if_else(
      is_eligible_independent_board,
      score_independent_board / possible_independent_board,
      NA_real_
    ),
...
form990_scores <- form990_scores %>%
  select(
    ein_rollup, rating_size, is_donor_supported,
    # ----------------------------
    # Eligibility flags
